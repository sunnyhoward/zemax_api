...
matplotlib.use('agg')
from shack_hartmann.int_to_zern.Utils.functions import pred_to_phasemap, DataGenerator, plot_training, normalize_data, rmse, psnr
...
def test_set_analysis(test_data,test_labels,model):
...
def plot_zernikes(preds,test_labels,allele,path,n_plots):
    ...
    if len(preds[0]) == 36:
         ...
         for i in range(n_plots):
             ax[i].plot(preds[allele[i]],cmap='RdYlBu');ax[i].set_xticks([]);ax[i].set_yticks([])
             ax[i].plot(test_labels[allele[i]],cmap='RdYlBu');ax[i].set_xticks([]);ax[i].set_yticks([])
...
if __name__ == '__main__':
    ...
    test_labels = np.array(hf['zernikes'])
    test_data=np.array(hf['spots'])
    hf.close()

    model = tf.keras.models.load_model(path+'/ML/int_to_zern/model_data/vgg_small.h5')
    
    new_dir = path+'/ML/int_to_zern/model_data/'+model.name
    if not os.path.exists(new_dir):
        os.makedirs(new_dir)

    nplots = 8

    predictions = test_set_analysis(test_data,test_labels,model)

    N = max(test_data.shape[1:])
